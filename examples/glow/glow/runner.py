...
class GlowRunner(skeltorch.Runner):
    ...
    def conf_get(self, *args):
        return self.experiment.configuration.get(*args)

    def init_model(self, device):
        self.model = GlowModel(
            num_channels=3,
            num_blocks=self.conf_get('model', 'num_blocks'),
            num_flows=self.conf_get('model', 'num_flows'),
            squeezing_factor=self.conf_get(
                'model', 'squeezing_factor'
            ),
            permutation_type=self.conf_get('model', 'permutation_type'),
            coupling_type=self.conf_get('model', 'coupling_type'),
            num_filters=self.conf_get('model', 'num_filters'),
            kernel_size=self.conf_get('model', 'kernel_size')
        ).to(device)

    def init_optimizer(self, device):
        self.optimizer = torch.optim.Adam(
            params=self.model.parameters(),
            lr=self.conf_get('training', 'lr')
        )

    def init_others(self, device):
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=self.optimizer,
            patience=self.conf_get('training', 'lr_scheduler_patience')
        )

    def train_step(self, it_data, device):
        x = it_data[0].to(device)
        x = GlowRunner.add_noise(x, self.conf_get('data', 'pixel_depth'))
        z, log_det = self.model(x)
        return GlowRunner.compute_loss(
            z, log_det, self.conf_get('data', 'pixel_depth')
        )
    ...
    def train_early_stopping(self):
        ...
        return self.counters['epoch'] - best_epoch > self.conf_get(
    ...
    def test(self, epoch, device):
        ...
        z = torch.randn((5, self.conf_get('data', 'image_size') ** 2 * 3)).to(
            device
        )
